...
1. **Screenshot** — Uruchom aplikację i zrób zrzut ekranu nowego ficzera.
2. **Zapis** — Zapisz obraz w `docs/images/{feature_name}.png`.
3. **README** — Dodaj nową sekcję lub zaktualizuj istniejącą w `README.md` korzystając z `<details>` i `<summary>`.
4. **Status** — Zaktualizuj tabelę statusu funkcjonalności.
